...
const Button: React.FC<ButtonProps> = ({ 
  ...
  const baseStyles = "inline-flex items-center justify-center font-bold tracking-wide transition-all duration-300 transform rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-brand-black no-underline";
  ...
  // Lógica de URL com utms
  const getUrl = () => {
    const linkBase = 'https://pay.hotmart.com/T104984177E?off=nlopn7u6&bid=1782595646491';
    const params = typeof window !== 'undefined' ? window.location.search : '';
    return params ? `${linkBase}&${params.startsWith('?') ? params.substring(1) : params}` : linkBase;
  };

  // Retornamos um <a> para ser detectado nativamente pelo GTM como linkClick
  return (
    <a 
      id={id}
      href={props.href || getUrl()}

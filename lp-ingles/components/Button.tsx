...
const Button: React.FC<ButtonProps> = ({ 
  ...
  const { checkoutUrl } = useVariant();
  const parametros = typeof window !== 'undefined' ? window.location.search : '';
  const finalHref = parametros
    ? `${checkoutUrl}&${parametros.startsWith('?') ? parametros.substring(1) : parametros}`
    : checkoutUrl;
